...
# ╔═╡ 82ff3ce4-4f3c-414d-8f6a-57cfec9fe65b
...
ox1 = let
	...
	ox1.indexlag21 = panellag(:StringencyIndex, ox1, :CountryCode, :t, 21)
...
let
	chisadza_lm = reg(ox1, @formula(deathspm ~ StringencyIndex + deathspmlag + 
	...
	chisadza_log = reg(ox1, @formula(logdeathspm ~ StringencyIndex + log(deathspmlag+1e-6) + 
	log(GDPpc) + Diabetes_prevalence + Hospital_bed_per1000 + Old_age_DR
), Vcov.cluster(:CountryCode))
...
let 
	...
	log(casesp100klag21+1) + casesp100klag21 + deathslag21 +
		log(GDPpc) + Diabetes_prevalence + Hospital_bed_per1000 + Old_age_DR
	), Vcov.cluster(:CountryCode))
	...
	growth2 = reg(ox1, @formula(deathgrowth ~ indexlag21 + deathslag21 + log(deathslag21+1e-6) + 
		log(GDPpc) + Diabetes_prevalence + Hospital_bed_per1000 + Old_age_DR
	), Vcov.cluster(:CountryCode))
	growth3 = reg(ox1, @formula(deathgrowth ~ indexlag21 + deathslag21 + log(deathslag21+1e-6) + casesp100klag21 + log(casesp100klag21 + 1) +
		log(GDPpc) + Diabetes_prevalence + Hospital_bed_per1000 + Old_age_DR
	), Vcov.cluster(:CountryCode))
...
# ╔═╡ 584e79e9-2df9-458a-aca5-e230de4d951b
mean(skipmissing(ox1.deathgrowth))

# ╔═╡ 95f2b750-876b-44f4-a457-f3089ccfdec2
let 
	df = #filter(r->(!ismissing(r.ConfirmedDeaths) && r.ConfirmedDeaths≥1000), ox1)
		filter(r->(r.date >= Date(2020,3,01)), ox1)
	fig = Figure()
	first = true
	for g in groupby(df, [:CountryCode])
		if first
			lines(fig[1,1], g.t, g.StringencyIndex)
			first = false
		else
			lines!(fig[1,1], g.t, g.StringencyIndex)
		end
	end
	scatter(fig[2,1], df.t, df.deathgrowth)
	fig
end

# ╔═╡ 54fb7fbc-ba4b-45d9-b8d4-f089ba1a49d3
hist(collect(skipmissing(ox1.deathgrowth)),bins=100)

# ╔═╡ 50fdce71-2d0f-4d21-a2af-21ceec8f9695
let 
	g = groupby(ox1, [:CountryCode])
	df = combine(g, :StringencyIndex => mean, :ConfirmedDeaths=>last, :Population=>last)
	fig = Figure()
	df.deaths_per_million = df.ConfirmedDeaths_last./df.Population_last*1e6
	scatter(fig[1,1],df.StringencyIndex_mean, df.deaths_per_million)
	for r in eachrow(df)
		if !ismissing(r.StringencyIndex_mean) && !ismissing(r.deaths_per_million)
			text!(fig[1,1],r.CountryCode, position=Point(r.StringencyIndex_mean, r.deaths_per_million))
		end
	end
	fig
	#reg(df, @formula(ConfirmedDeaths_last ~ StringencyIndex_sum))
	#df
end
...
git-tree-sha1 = "d2b82dcf3624ed1e2a1e5c126aa317d163243168"
uuid = "9d5cd8c9-2029-5cab-9928-427838db53e3"
version = "1.6.3"

[[deps.FixedEffects]]
deps = ["DataAPI", "LinearAlgebra", "Printf", "Requires", "StatsBase"]
git-tree-sha1 = "cf454d95fe0ebac4c97d1bd5f01b080cec533c68"
...
[[deps.Vcov]]
deps = ["Combinatorics", "DataAPI", "LinearAlgebra", "StatsBase", "Tables"]
git-tree-sha1 = "85f4c3c95d9a4d3cce71cb6c58c3a5e496edb5af"
...
[[deps.WorldBankData]]
deps = ["DataFrames", "HTTP", "JSON"]
git-tree-sha1 = "5b0ac92354f25754de6cbc3277bb8d31fb3eac81"
uuid = "2ff30676-a39f-58b2-ad74-5d1012d57b9b"
version = "0.4.1"
